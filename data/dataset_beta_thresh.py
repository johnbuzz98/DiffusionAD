...
class MVTecTestDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sample = {
            ...
            "mask": mask,
            "idx": idx,
            "type": img_path[len(self.root_dir) : -8],
            "file_name": base_dir + "_" + file_name,
        }
...
class MVTecTrainDataset(Dataset):
    ...
    def randAugmenter_anomaly(self):
        aug_ind = np.random.choice(np.arange(len(self.augmenters_anomaly)), 2, replace=False)
        aug = iaa.Sequential([self.augmenters_anomaly[aug_ind[0]], self.augmenters_anomaly[aug_ind[1]]])
        return aug

    def randAugmenter_mask(self):
        aug_ind = np.random.choice(np.arange(len(self.augmenters_mask)), 1, replace=False)
        aug = iaa.Sequential(
            [
                self.augmenters_mask[aug_ind[0]],
            ]
        )
        return aug
    ...
    def perlin_synthetic(self, image, thresh, anomaly_source_path, cv2_image, thresh_path):
        ...
        if no_anomaly > 0.5:
            ...
            if self.classname in texture_list:  # only DTD
                ...
                img_object_thr = anomaly_img_augmented.astype(np.float32) * object_perlin / 255.0
                ...
                if texture_or_patch > 0.5:  # >0.5 is DTD
                    ...
                    wi = np.split(anomaly_image, range(gird_width, width, gird_width), axis=1)
                    wi1 = wi[::2]
                    random.shuffle(wi1)
                    wi2 = wi[1::2]
                    random.shuffle(wi2)
                    ...
                    hi = np.split(width_cut_image, range(gird_high, high, gird_high), axis=0)
                    random.shuffle(hi)
                    hi1 = hi[::2]
                    random.shuffle(hi1)
                    hi2 = hi[1::2]
                    ...
                    img_object_thr = mixer_cut_image.astype(np.float32) * object_perlin / 255.0

            beta = torch.rand(1).numpy()[0] * 0.6 + 0.2
            augmented_image = (
                image * (1 - object_perlin) + (1 - beta) * img_object_thr + beta * image * (object_perlin)
            )
...
class VisATestDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if base_dir == "good":
            ...
            upper_dir_path = os.path.dirname(os.path.dirname(dir_path))
            mask_path = os.path.join(upper_dir_path, "ground_truth")
            # mask_path = os.path.join(dir_path, "../../ground_truth/")
            # mask_path = os.path.join(mask_path, base_dir)
